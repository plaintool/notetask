...
const
  Brackets: array[0..17] of string = ('- [x]', '- [X]', '- [ ]', '- []', '-[x]', '-[X]', '-[ ]', '-[]', '[x]',
    '[X]', '[ ]', '[]', 'x ', '-x ', '- x ', 'X ', '-X ', '- X ');
  BracketsDone: array[0..11] of string = ('- [x]', '- [X]', '-[x]', '-[X]', '[x]', '[X]', 'x ', '-x ', '- x ', 'X ', '-X ', '- X ');
...
uses stringhelper, stringshelper;
...
var
  ...
  begin
    ...
    begin
      ...
      if (Part[0].Trim.ToFloatTry(Result1.FAmount)) then
      begin
        Result1.FText := Part.JoinArrayText(1, Separator);
      ...
      if (Part[0].Trim.ToDateTimeISOTry(Result1.FDate)) then
      begin
        // Extract and trim the amount string
        if (Part[1].Trim.ToFloatTry(Result1.FAmount)) then
          Result1.FText := Part.JoinArrayText(2, Separator)
        else
        begin
          Result1.FText := Part.JoinArrayText(1, Separator);
          Result1.FAmount := 0;
...
begin
  ...
  if (Length(PartNote) >= 2) then
    for i := 0 to High(PartNote) do
      ...
      begin
        CompletedStr := PartNote.JoinArrayText(0, cm, i).TrimRight(cm);
        ...
        Result.FNote := PartNote.JoinArrayText(i + 1, cm);
  ...
  if (CompletedStr = string.Empty) then
    CompletedStr := PartNote.JoinArrayText(0, cm);
  ...
  Result.FDone := PartDate[0].StartsWithStrings(BracketsDone);
  PartDate[0] := PartDate[0].RemoveStrings(Brackets);

  // Checks if the task is completed
  CompletedStr := CompletedStr.RemoveStrings(Brackets);

  if (CompletedStr.RemoveSpaceChar.ToFloatTry(Result.FAmount)) then
  begin
    Result.FAmountOriginal := CompletedStr;
  end
  else
  if (CompletedStr.ToDateTimeISOTry(Result.FDate)) then
  begin
  ...
  begin
    PartSpace := CompletedStr.SplitByFirstSpaces(2);
  ...
  CleanedText := Trim(Result.FText.RemoveBacktickBlocks);
  ...
  begin
    ...
    Result.FText := Result.FText.RemoveFirstSubstring(ar);
    Result.FText := Result.FText.RemoveFirstSubstring(ar, True);
  ...
  begin
    ...
    Result.FText := Result.FText.RemoveFirstSubstring(st);
    Result.FText := Result.FText.RemoveFirstSubstring(st, True);
  end
  else
  if Result.FText.TrimLeft.StartsWithBracketAZ then
    Result.FStar := True
  else
    Result.FStar := False;

  // Calculate indent level: 2 spaces = 1 level, odd space kept as text
  Result.FText := Result.FText.ExtractIndent(Result.FIndentLevel);
...
begin
  ...
  TextString := Task.FText.ReplaceLineBreaks;
  NoteString := Task.FNote.ReplaceLineBreaks;

  // Restore indent inside task text (2 spaces per level)
  if Task.FIndentLevel > 0 then
    TextString.AddIndent(Task.FIndentLevel);

  // Add Tags to TextString
  if (Col = 0) and (Task.Tags.Count > 0) then
    TextString := TextString + Task.Tags.ToBacktickString(TextString <> string.Empty);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        Value.SplitStringByComment(TabName, TabHint);
  ...
  begin
    ...
    begin
      if (GetGroupHint(i) <> string.Empty) then
        Result.Add(GetGroupName(i) + ' // ' + GetGroupHint(i).ReplaceLineBreaks)
      else
    ...
    begin
      ...
      begin
        ...
        if (Rect.Left = COL_TASK) and (GetTask(i).FIndentLevel > 0) then
          RowText.AddIndent(GetTask(i).FIndentLevel);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              pText := pText.RemoveFirstSubstring(ar);
              pText := pText.RemoveFirstSubstring(ar, True);
              Archive := True;
            end;
            Star := False;
            if pText.StartsWith(st) and pText.EndsWith(st) then
            begin
              pText := pText.RemoveFirstSubstring(st);
              pText := pText.RemoveFirstSubstring(st, True);
              Star := True;
            end;
            if Rect.Width > 0 then pText := pText + GetTask(i).Tags.ToBacktickString(pText <> string.Empty);
        ...
        Row2 := (pText + ifthen((pNote = string.Empty) or pNote.StartsWithChar, string.Empty, sp) + pNote);
